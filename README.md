...
- **Apariencia**: tema claro, oscuro o automático (según el sistema operativo), sin parpadeo al cargar la página — combinado con una paleta de color independiente (Frakta, Océano, Lila, Minimalista) que tampoco parpadea al cargar. El logo y el gradiente de marca se mantienen iguales en las cuatro.
...
- **Responsive de verdad**: sidebar colapsable en escritorio, barra de navegación inferior de 4 slots en mobile (los tres módulos de consulta diaria + un "Más" con el resto). Todos los botones, inputs y controles alcanzan el mínimo de 44×44px recomendado para touch en mobile, los formularios se abren como hojas deslizantes desde abajo en vez de modales centrados, y el layout respeta el "notch"/la barra de gestos de iOS y Android.
...
- **`Dialog` y `Sheet` son el mismo primitivo de Radix por dentro.** Eso permitió armar un `ResponsiveDialog` que elige centrado (desktop) o deslizante desde abajo (mobile) según el viewport, y migrar los ~15 formularios existentes con un cambio de una sola línea por archivo (un alias en el import), sin tocar el JSX de ninguno.
