...
Post-Fase 5 (pedido del usuario, fuera del roadmap original): **Horario** ahora acepta bloques que no son una materia — `BloqueHorario.materiaId` es `string | null`, con `titulo`/`icono` propios cuando es una "actividad libre" (ej. "Gimnasio" con ícono de pesas, `src/features/horario/iconosActividad.ts`, 16 íconos curados); `BloqueForm.tsx` tiene un toggle Materia/Actividad y ya no bloquea la creación de bloques cuando no hay materias cargadas. **Proyectos** dejó de ser un valor invisible: `Tareas.tsx` suma una pestaña "Proyectos" con CRUD completo (`ProyectoForm.tsx`, `ProyectosList.tsx`, borrado en cascada que desvincula tareas en vez de borrarlas) y `TareaForm.tsx` siempre muestra el campo Proyecto con un ítem "Crear proyecto nuevo…" que abre un diálogo anidado y auto-selecciona el proyecto recién creado. **Ajustes**: recordatorios y almacenamiento persistente ahora son cancelables de verdad vía un switch (`ConfigApp.recordatoriosActivos` / `almacenamientoPersistenteActivo`, con nota honesta de que el navegador no permite revocar la persistencia ya otorgada por API) y se agregó una "Zona de peligro" con reinicio total de datos (`src/db/reiniciar.ts`, confirmación por texto "REINICIAR"). Al implementar el toggle apareció y se corrigió una race condition real: `actualizarConfig()` hacía lectura+escritura sin transacción, así que togglear dos switches casi al mismo tiempo pisaba un cambio con el otro — ahora está envuelto en `db.transaction('rw', db.config, ...)`. También se agregó el recordatorio de backup de 14 días que ya estaba en el blueprint pero no implementado (punto dorado en el ícono de Ajustes, `src/lib/backupRecordatorio.ts` + `uiStore.recordatorioBackupPendiente`).
