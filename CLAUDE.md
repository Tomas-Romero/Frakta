...
Fase 0 completa: scaffold Vite+React+TS, Tailwind v4 + shadcn/ui (Radix), capa Dexie (`src/db/db.ts`) en `schemaVersion: 1`, `migrarBackup()` en `src/db/backup.ts`, sidebar de 3 capas + Dashboard de solo lectura, e import/export de backup JSON validado con Zod (`src/db/backupSchema.ts`) — rechazo entero ante un archivo inválido, verificado a mano. Fase 1 (MVP académico) sin empezar. Actualizar esta línea (o simplemente mirar los checkboxes de `docs/ROADMAP.md`) al retomar el proyecto en una nueva sesión de Claude Code.
